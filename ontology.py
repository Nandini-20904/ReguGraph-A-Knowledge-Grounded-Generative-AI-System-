...
def normalize_doc_id(raw_doc_id: str):
    rid = raw_doc_id.lower()
    if "outsourcing" in rid:
        return "outsourcing_directions_2025"
    if "digital" in rid:
        return "digital_lending"
    if "expected credit loss" in rid or "ecl" in rid:
        return "ecl_framework"
    return raw_doc_id.replace(" ", "_").lower()

# ---------------------
# LLM Classifier
# ---------------------
class LLMClassifier:
    def __init__(self, model="llama-3.3-70b-versatile", key_env="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"):
...
TOPIC_DOMAIN_MAP = {
    ...
    # Outsourcing Risk (NEW FIX)
    "Outsourcing_Risk_Governance": ["outsourcing_risk", "outsourcing_directions"],
    "Outsourcing_Due_Diligence": ["outsourcing_risk", "outsourcing_directions"],
    "Outsourcing_Materiality": ["outsourcing_risk", "outsourcing_directions"],
    "Outsourcing_Contractual_Controls": ["outsourcing_risk", "outsourcing_directions"],
    "Outsourcing_Monitoring_Audit": ["outsourcing_risk", "outsourcing_directions"],
    "Outsourcing_Exit_Strategy": ["outsourcing_risk", "outsourcing_directions"],
...
def extract(chunks_dir, topic_kw_file, canonical_file, out_dir, llm_mode="hybrid"):
    ...
    for file in tqdm(list(Path(chunks_dir).glob("*_chunks.json"))):
        ...
        raw_doc_id = doc_chunks[0].get("doc_id", file.stem)
